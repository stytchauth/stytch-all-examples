import { createMcpServer } from './SprintPlannerMCP';
import { SprintPlannerAPI } from './SprintPlannerAPI';
...
const authenticateTokenAuthMiddleware = B2B.authenticateOAuthToken({
  onError: (c, err) => {
    console.error(err);
...
export default new Hono<{ Bindings: Env }>()
  ...
  // Mount the Sprint Planner API underneath us
  .route('/api', SprintPlannerAPI)
