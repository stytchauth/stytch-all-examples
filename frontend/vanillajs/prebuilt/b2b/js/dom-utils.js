...
export function updateLoginUI(eventType) {
  ...
  if (eventType === "B2B_MAGIC_LINK_EMAIL_DISCOVERY_SEND") {
...
export function setupOrganizationsEventListeners(handlers) {
  const orgCreateForm = getElementById("org-create-form");
  const orgNameInput = getElementById("org-name-input");

  addEventListener(orgCreateForm, "submit", handlers.handleCreateOrg);
  addEventListener(orgNameInput, "input", handlers.updateButtonStates);
}

export function updateOrganizationsUI(eventType) {
  const orgsTextBox = getElementById("orgs-text-box");
  const orgCreateTextBox = getElementById("org-create-text-box");
  const createOrgContainer = getElementById("create-org-container");
  const stytchSDK = getElementById("stytch-sdk");
  const yourOwnUIBadge = getElementById("your-own-ui-badge");

  if (eventType === "B2B_DISCOVERY_ORGANIZATIONS_CREATE") {
    toggleElementVisibility(orgsTextBox, false);
    toggleElementVisibility(orgCreateTextBox, true);
    toggleElementVisibility(createOrgContainer, true);
    toggleElementVisibility(stytchSDK, false);
    toggleElementVisibility(yourOwnUIBadge, true);
  }
}

export function updateCreateOrgButton(disabled, isLoading) {
  const createOrgBtn = getElementById("create-org-btn");

  setElementDisabled(createOrgBtn, disabled);
...
export function displaySessionData(member, organization) {
  const emailDisplay = getElementById("email-display");
  const memberIdDisplay = getElementById("member-id-display");
  const orgNameDisplay = getElementById("org-name-display");

  setElementText(emailDisplay, member.email_address || "N/A");
  setElementText(memberIdDisplay, member.member_id || "N/A");
  setElementText(orgNameDisplay, organization.organization_name || "N/A");
}
