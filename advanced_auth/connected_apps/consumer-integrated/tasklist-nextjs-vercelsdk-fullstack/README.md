# Consumer Integrated: Task List Next.js Fullstack

This is a full-stack Next.js backend for the Task List example.
This backend uses the Vercel MCP SDK Adapter for the MCP server.

This example contains its own frontend using Next.js
...
To start the API and MCP server, run the following command. The server will run on localhost:3000.

```bash
# if running from this directory
yarn dev
# if running from the `mcp-examples` directory
yarn workspace @mcp-examples/tasklist-nextjs-vercelsdk-backend dev
```
