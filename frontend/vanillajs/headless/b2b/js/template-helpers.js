// Simple template helper functions to keep HTML out of core logic

/**
 * Creates an organization button element
 * @param {string} orgId - Organization ID
 * @param {string} orgName - Organization name
 * @returns {HTMLElement} - Button element
 */
export function createOrgButton(orgId, orgName) {
    const button = document.createElement('button');
    button.className = 'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*="size-"]):size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-blue-500 focus-visible:ring-blue-500/50 focus-visible:ring-[3px] bg-white text-black shadow-sm hover:bg-gray-100 h-12 px-4 py-2 has-[>svg]:px-3 border border-gray-200 w-full';
    button.setAttribute('data-org-id', orgId);
    ...
    button.innerHTML = `
        ...
        <p class="flex-1 text-left text-body1 font-bold">${orgName}</p>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
