...
function formatSingleTicketResponse(
  description: string,
  ticket: Ticket | null,
) {
  return {
...
export function createMcpServer(ticketService: TicketService): McpServer {
  ...
  server.tool(
    ...
    "Create a new ticket",
    {
      ...
      description: z
        .string()
        .optional()
        .describe("Optional description of the ticket"),
    },
  ...
  server.tool(
    ...
    "Update the status of a ticket",
    {
      ...
      status: z
        ...
        .describe("The new status for the ticket"),
    },
        ...
        content: [
          {
            type: "text" as const,
            text: success
              ? `Success! Deleted ticket ${ticket_id}`
              : `Failed! Ticket ${ticket_id} not found`,
          },
        ],
      };
    },
  ...
  server.tool(
    ...
    "Search tickets with filters",
    {
      status: z
        .string()
        .optional()
        .describe("Filter by status (backlog, in-progress, review, done)"),
      assignee: z.string().optional().describe("Filter by assignee name"),
      title_contains: z
        .string()
        .optional()
        .describe("Filter by title containing text"),
    },
  ...
  server.tool(
    ...
    async () => {
      ...
      return formatStatisticsResponse(
        "Retrieved ticket statistics",
        statistics,
      );
    },
  );
