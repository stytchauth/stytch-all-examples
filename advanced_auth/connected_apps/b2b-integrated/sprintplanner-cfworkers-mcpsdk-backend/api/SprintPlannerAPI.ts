...
export const SprintPlannerAPI = new Hono<{ Bindings: Env }>()
  ...
  .get("/healthcheck", async (c) => {
    ...
    if (!c.env.STYTCH_PROJECT_ID) {
      errors.push({
        ...
        description:
          "Your Stytch project ID (e.g., project-test-6c20cd16-73d5-44f7-852c-9a7e7b2ccf62)",
      });
    }
    ...
    if (!c.env.STYTCH_DOMAIN) {
      errors.push({
        ...
        description:
          "Your Stytch domain (e.g., https://cname-word-1234.customers.stytch.dev)",
      });
    }
      ...
      return c.json(
        {
          ...
          errors,
          message:
            "Backend configuration is incomplete. Add missing variables to .dev.vars file.",
          configFile: ".dev.vars",
        },
        500,
      );
  ...
  .post("/tickets/:id/status", async (c) => {
    ...
    const tickets = await sprintPlannerService(c.env, orgId).updateTicketStatus(
      id,
      body.status,
    );
  ...
  .get("/tickets/search", async (c) => {
    ...
    const tickets = await sprintPlannerService(c.env, orgId).searchTickets(
      params,
    );
    return c.json({ tickets });
  })
  ...
  .get("/tickets/statistics", async (c) => {
    ...
    const orgId = session.organization_id;
    if (!orgId) return c.json({ error: "No organization in session" }, 401);
    const stats = await sprintPlannerService(
      c.env,
      orgId,
    ).getTicketStatistics();
    return c.json(stats);
