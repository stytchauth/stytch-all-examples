...
export function updateCreateButtonVisibility(hasSession) {
  ...
  if (showCreateOrg) {
    ...
    // Only show the OR divider if there are organizations to display
    const orgsList = getElementById("orgs-list");
    if (orgsList && orgsList.children.length > 0) {
      addOrgDivider();
    }
  } else {
...
export function setupViewSessionEventListeners(handlers) {
  ...
  const switchOrgsBtn = getElementById("switch-orgs-btn");
  ...
  addEventListener(switchOrgsBtn, "click", handlers.handleSwitchOrgs);
